...
/**
 * Comando para adicionar un registro de la tabla: mus_roles
 * @author SpyroFrameWork
 * @copyright Spyro Solutions
 */
class CmdAddMus_roles {
	...
	function execute(){
		...
		if(($mus_roles__ROL_ID != NULL) && ($mus_roles__ROL_ID != "") && ($mus_roles__ROL_NOMBRE != NULL) && ($mus_roles__ROL_NOMBRE != "")){
			...
			$mus_roles__ROL_NOMBRE = $obj_datatype->formatString($mus_roles__ROL_NOMBRE);
			$mus_roles_manager = Application::getDomainController('Mus_rolesManager'); 
			$message = $mus_roles_manager->addMus_roles($mus_roles__ROL_ID,$mus_roles__ROL_NOMBRE); 
